-- Fix cadence task scheduling to use company timezone
-- Tasks should be scheduled based on company's local time, not UTC
...
DECLARE
    ...
    v_current_time_in_tz TIMESTAMP WITH TIME ZONE;
BEGIN
    ...
    -- Convert to company timezone for date calculation
    v_due_date := (p_started_at AT TIME ZONE v_company_timezone +
    ...
    -- Combine date and time in company timezone
    v_calculated_datetime := (v_due_date::TEXT || ' ' || v_due_time::TEXT)::TIMESTAMP;
    v_calculated_datetime := v_calculated_datetime AT TIME ZONE v_company_timezone;

    -- Get current time in company timezone
    v_current_time_in_tz := NOW() AT TIME ZONE v_company_timezone;

    -- If the calculated datetime is in the past (in company's local time), bump to next day
    IF v_calculated_datetime < v_current_time_in_tz THEN
