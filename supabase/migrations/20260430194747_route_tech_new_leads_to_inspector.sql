-- Route TechRoutes new-leads (lead_source='technician') to an inspector via
-- zip_code_groups + property_type, regardless of whether a service plan was
-- picked or whether that plan requires a custom quote.
--
-- Rule:
--   - Tech upsell: client pre-assigns to the submitting tech, so the trigger
--     bails at the assigned_to-null gate (no change there).
--   - Tech new-lead: comes in unassigned and runs through the full inspector
--     resolution path — even when no plan is selected and even when the
--     auto_assign_custom_quote_leads company setting is off.
--   - Other sources: unchanged from prior migration. Still gated on
--     selected_plan_id + requires_quote + auto_assign_custom_quote_leads.
...
BEGIN
  ...
  -- Non-tech sources still need a custom-quote plan + the company opt-in.
  -- Tech-source leads bypass both gates so every TechRoutes new-lead routes.
  IF NOT v_is_tech_lead THEN
    IF NEW.selected_plan_id IS NULL THEN RETURN NEW; END IF;
  ...
  -- Cached branch on the linked service_address takes precedence over the
  -- ZIP fast-path. Only honor entries that have actually been resolved
  ...
  -- Fallback: zip-only so the lead still gets assigned if nobody in the zip
  -- group is tagged for this property_type.
