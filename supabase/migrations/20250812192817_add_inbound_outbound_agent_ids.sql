-- Add inbound and outbound agent ID settings to existing companies
...
SELECT 
    ...
    '',
    'string',
    'Retell AI agent ID for handling inbound calls'
FROM companies
...
SELECT 
    ...
    '',
    'string',
    'Retell AI agent ID for handling outbound calls from form submissions'
FROM companies
ON CONFLICT (company_id, setting_key) DO NOTHING;

-- Update the trigger function to include new agent settings for new companies
...
BEGIN
    ...
    VALUES 
        ...
        (NEW.id, 'retell_agent_id', '', 'string', 'Company-specific Retell AI agent ID (legacy, use inbound/outbound specific IDs)'),
