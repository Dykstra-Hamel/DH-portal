-- Move check_service_area_coverage function back to public schema
-- PostgREST only allows access to public and graphql_public schemas

-- Drop the function from extensions schema
DROP FUNCTION IF EXISTS extensions.check_service_area_coverage(uuid, numeric, numeric, text);

-- Create the function in public schema with proper PostGIS access
...
COMMENT ON FUNCTION public.check_service_area_coverage(uuid, numeric, numeric, text) IS 'Check if a geographic point or zip code falls within any active service area for a company - Moved back to public schema for PostgREST access 2025-08-22';