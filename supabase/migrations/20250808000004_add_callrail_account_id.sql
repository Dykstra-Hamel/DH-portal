-- Add CallRail Account ID to company settings

-- Update the company creation trigger to include CallRail Account ID
...
BEGIN
    ...
    VALUES 
        ...
        (NEW.id, 'callrail_account_id', '', 'string', 'CallRail Account ID to specify which account to use')
...
-- Add CallRail Account ID setting to existing companies that don't have it
INSERT INTO company_settings (company_id, setting_key, setting_value, setting_type, description)
SELECT 
    id,
    'callrail_account_id',
    '',
    'string',
    'CallRail Account ID to specify which account to use'
FROM companies 
WHERE NOT EXISTS (
    SELECT 1 FROM company_settings 
    WHERE company_settings.company_id = companies.id 
    AND company_settings.setting_key = 'callrail_account_id'
);