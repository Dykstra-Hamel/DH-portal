...
BEGIN
    ...
    IF NEW.signed_at IS NOT NULL AND (OLD.signed_at IS NULL OR OLD.signed_at != NEW.signed_at) THEN
        ...
        IF v_lead.assigned_to IS NOT NULL THEN
            ...
            SELECT COALESCE(SUM(
                CASE
                    WHEN qli.discount_id IS NOT NULL THEN
                        CASE
                            WHEN d.discount_type = 'percentage' THEN
                                qli.price - (qli.price * d.discount_value / 100)
                            ELSE
                                qli.price - d.discount_value
                        END
                    ELSE
                        qli.price
                END
            ), 0) INTO v_quote_total
            FROM quote_line_items qli
            LEFT JOIN discounts d ON qli.discount_id = d.id
