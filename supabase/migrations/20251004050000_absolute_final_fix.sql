-- ABSOLUTE FINAL FIX
-- Drop and recreate with explicit CASCADE to ensure all dependencies are handled
...
BEGIN
    RAISE NOTICE 'Creating task for lead % with step %', p_lead_id, p_cadence_step_id;
    ...
    RAISE NOTICE 'About to insert task with related_entity_type = leads';

    -- INSERT WITH 'leads' PLURAL
    INSERT INTO tasks (
        company_id,
        title,
        status,
        ...
        v_due_date,
        v_due_time,
        'leads',  -- PLURAL!!! NOT 'lead'
        p_lead_id,
        p_cadence_step_id
    )
    RETURNING id INTO v_task_id;

    RAISE NOTICE 'Task created with ID %', v_task_id;
