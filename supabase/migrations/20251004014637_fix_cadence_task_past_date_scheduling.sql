-- Fix cadence task scheduling to prevent tasks from being scheduled in the past
-- When a cadence is started, if the calculated task time is in the past, bump it to the next day
...
BEGIN
    ...
    -- Cast to timestamptz to properly compare with p_started_at
    v_calculated_datetime := (v_due_date::TEXT || ' ' || v_due_time::TEXT)::TIMESTAMP WITH TIME ZONE;

    -- Debug: Log the values
    RAISE NOTICE 'p_started_at: %, v_calculated_datetime: %, comparison: %',
        p_started_at, v_calculated_datetime, (v_calculated_datetime < p_started_at);

    -- If the calculated datetime is in the past compared to when cadence started, bump to next day
    IF v_calculated_datetime < p_started_at THEN
        RAISE NOTICE 'Bumping date from % to next day', v_due_date;
...
COMMENT ON FUNCTION create_task_for_cadence_step IS 'Creates a task for a specific cadence step with calculated due date and time. If the calculated time is in the past, it schedules for the next day to ensure tasks are always in the future.';
