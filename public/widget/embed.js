 ...
 * Source files: widget-state.js, widget-utils.js, widget-styles.js, widget-ui.js, widget-logic.js, widget-captcha.js, widget-forms.js, widget-api.js, embed-main.js
 */
...
(function () {
    ...
    const displayError = () => {
      try {
        ...
        errorTitle.style.cssText =
          'font-weight: bold; margin: 0 0 10px 0; font-size: 16px;';
  ...
  const scriptTag =
    ...
    (function () {
      ...
      let foundScript = document.querySelector(
        'script[data-script-id="dh-widget"]'
      );
      // Try to find by data-container-id attribute (React integration)
      if (!foundScript)
        foundScript = document.querySelector('script[data-container-id]');
  ...
  if (!scriptTag) {
    showErrorState(
      ...
      'Could not determine script tag reference'
    );
    return;
  }
  ...
  const getPestTypeDisplay = (pestType, context = 'default') => {
    if (!pestType) {
      const fallback = context === 'comparison' ? 'your pest issue' : 'pests';
      return fallback;
    }
  ...
  const updateDynamicText = async () => {
    ...
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        const planComparisonHeader = document.getElementById(
        ...
        if (planComparisonHeader && widgetState.formData.pestType) {
          ...
          const selectedPestOption = pestOptions.find(
            option => option.value === widgetState.formData.pestType
          );

          if (
            selectedPestOption &&
            selectedPestOption.plan_comparison_header_text
          ) {
            // Use custom header text and replace [pest type] placeholder
            const customHeaderText =
              selectedPestOption.plan_comparison_header_text.replace(
                /\[pest type\]/g,
                getPestTypeDisplay(widgetState.formData.pestType, 'comparison')
              );
        ...
        if (completionDescription && widgetState.widgetConfig?.successMessage) {
          completionDescription.textContent =
            widgetState.widgetConfig.successMessage;
        }
        ...
        const addressCityRefs = document.querySelectorAll('.address-city-ref');
  ...
  const replaceStepVariables = (text, pestType, recommendedPlan) => {
    ...
    const initialPrice = recommendedPlan?.initial_price
      ? `$${recommendedPlan.initial_price}`
      : '$';
    const recurringPrice =
      recommendedPlan?.recurring_price && recommendedPlan?.billing_frequency
  ...
  const updateStepHeadings = () => {
    ...
    if (addressHeading && stepHeadings.address) {
      const processedText = replaceStepVariables(
        stepHeadings.address,
        pestType,
        recommendedPlan
      );
  ...
  const loadAddressImagery = async address => {
    ...
    try {
      ...
      const hasStreetView = await checkStreetViewAvailability(lat, lon, apiKey);
      ...
      if (hasStreetView) {
        ...
        try {
          ...
          console.warn(
            'Street View failed, trying satellite:',
            streetViewError
          );
  ...
  /* dh-pulse keyframes moved to consolidated animations section */
  ...
  /* Note: This duplicate .dh-image-loading .dh-loading-spinner definition removed - 
   consolidated with the one above */
  ...
  /* Responsive styling moved to consolidated responsive section below */

  /* Spinning Animation for Loading */
  /* dh-spin keyframes removed - consolidated with spin animation below */
  ...
  /* 480px media query styles moved to consolidated responsive section */
  ...
  .dh-form-btn-secondary:hover:not(:disabled) {
  ...
  /* All duplicate keyframes and scattered media queries moved to consolidated sections */
  ...
  /* Hidden pest options - will be visible but clipped by parent container */
  .dh-pest-option-hidden {
  /* These pests are still in the grid but will be clipped by the parent container's max-height */
  opacity: 1;
  transform: translateY(0);
  }

  /* Pest options transition for smooth reveal effect */
  .dh-pest-option {
  transition: opacity 0.3s ease, transform 0.3s ease;
  }

  /* Staggered reveal animation for pests */
  .dh-pest-option-revealing {
  animation: slideInFromBottom 0.4s cubic-bezier(0.4, 0, 0.2, 1) both;
  }
  ...
  /* View All Pests button container */
  ...
  #view-all-pests-button svg {
  display: none;
  }

  #view-all-pests-button:hover {
  color: ${secondaryDark};
  ...
  .dh-pest-selection.expanded {
    max-height: 1000px; /* Large enough to show all pests */
  }
  }
  ...
  /* First 768px media query styles moved to consolidated responsive section */
  ...
  /* Second 480px media query styles moved to consolidated responsive section */
  ...
  /* Base loading spinner - can be customized with CSS custom properties */
  .dh-loading-spinner {
  --spinner-size: 32px;
  --spinner-border: 3px;
  --spinner-margin: 0 auto 16px auto;
  
  width: var(--spinner-size);
  height: var(--spinner-size);
  border: var(--spinner-border) solid #e5e7eb;
  border-top: var(--spinner-border) solid ${primaryColor};
  border-radius: 50%;
  animation: spin 1s linear infinite;
  ...
  .dh-confirmation-popup.show {
  display: flex;
  opacity: 1;
  }

  .dh-confirmation-content {
  background: white;
  border-radius: 16px;
  padding: 32px;
  max-width: 400px;
  ...
  .dh-confirmation-popup.show .dh-confirmation-content {
  transform: scale(1);
  }

  .dh-confirmation-title {
  font-size: 20px;
  font-weight: 600;
  color: ${textColor};
  margin: 0 0 12px 0;
  ...
  .dh-confirmation-buttons {
  display: flex;
  gap: 12px;
  justify-content: center;
  }

  .dh-confirmation-button {
  padding: 12px 24px;
  border-radius: 8px;
  border: none;
  font-size: 14px;
  ...
  .dh-confirmation-button.cancel {
  background: ${primaryColor};
  color: white;
  }

  .dh-confirmation-button.cancel:hover {
  background: ${primaryDark};
  }

  .dh-confirmation-button.confirm {
  background: #f3f4f6;
  color: #374151;
  }

  .dh-confirmation-button.confirm:hover {
  background: #e5e7eb;
  }
  ...
  #dh-step-plan-comparison h2.dh-step-heading {
    font-size: 26px;
    line-height: 28px;
  ...
  /* ===================================================================
   CONSOLIDATED RESPONSIVE & MOBILE STYLES
   =================================================================== */
  ...
  /* Small tablets and large phones - 650px and below */
  @media (max-width: 650px) {
  .dh-plan-main {
    padding: 24px 16px;
  }
  ...
  * {
    ...
    if (modal && modalBody) {
      ...
      if (!modalWidgetCreated) {
        ...
        if (
          ...
          window.progressiveFormManager.shouldPromptToContinue(
            widgetState.recoveryData
          )
        ) {
          ...
          if (widgetState.recoveryData.source === 'cross-device') {
            ...
            setTimeout(() => {
              if (
                window.progressiveFormManager &&
                typeof window.progressiveFormManager.showContinuePrompt ===
                  'function'
              ) {
                window.progressiveFormManager.showContinuePrompt(
                  widgetState.recoveryData
                );
              }
  ...
  const createConfirmationPopup = () => {
    ...
    message.textContent =
      "Don't worry, your progress will be saved and you can continue another time.";
  ...
  const createButton = () => {
    ...
    button.textContent =
      ...
      (widgetState.widgetConfig &&
        widgetState.widgetConfig.welcomeButtonText) ||
      'Get Started';
    button.type = 'button';
  ...
  `
      ...
      case 'complete':
        ...
        const returnHomepageBtn = document.getElementById(
          'return-homepage-btn'
        );
  ...
  window.dhCaptcha = {
    init: async (provider, inputSiteKey) => {
      if (provider !== 'turnstile' || !inputSiteKey) {
        console.log(
          'Turnstile: Not initializing - provider:',
          provider,
          'siteKey present:',
          !!inputSiteKey
        );
  ...
  window.submitForm = async () => {
    ...
    const formData = {
      ...
      estimatedPrice: widgetState.formData.selectedPlan
        ? {
            ...
            max:
              widgetState.formData.selectedPlan.recurring_price ||
              widgetState.formData.selectedPlan.initial_price ||
              0,
            service_type:
              widgetState.formData.selectedPlan.plan_name ||
    ...
    try {
      ...
      try {
        const cfg =
          widgetState.widgetConfig && widgetState.widgetConfig.captcha;
        if (cfg && cfg.provider === 'turnstile' && cfg.siteKey) {
          ...
          if (submitBtn) {
            submitBtn.innerHTML =
              'Verifying security... <svg xmlns="http://www.w3.org/2000/svg" width="9" height="16" viewBox="0 0 9 16" fill="none"><path d="M1 14.9231L7.47761 7.99998L1 1.0769" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
          }
          ...
          const token =
            window.dhCaptcha && typeof window.dhCaptcha.getToken === 'function'
          ...
          if (submitBtn) {
            submitBtn.innerHTML =
              'Submitting... <svg xmlns="http://www.w3.org/2000/svg" width="9" height="16" viewBox="0 0 9 16" fill="none"><path d="M1 14.9231L7.47761 7.99998L1 1.0769" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
          }
        ...
        console.warn(
          'DH Widget: Captcha token acquisition failed',
          captchaError
        );
        ...
        if (submitBtn) {
          if (
            ...
            captchaError.message.includes('rate limit')
          ) {
            submitBtn.innerHTML =
              'Rate limited - retrying... <svg xmlns="http://www.w3.org/2000/svg" width="9" height="16" viewBox="0 0 9 16" fill="none"><path d="M1 14.9231L7.47761 7.99998L1 1.0769" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
          } else if (
            captchaError.message &&
            captchaError.message.includes('max retries')
          ) {
            submitBtn.innerHTML =
              'Security check failed <svg xmlns="http://www.w3.org/2000/svg" width="9" height="16" viewBox="0 0 9 16" fill="none"><path d="M1 14.9231L7.47761 7.99998L1 1.0769" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
            // Don't proceed with submission if captcha completely failed
            widgetState.isSubmitting = false;
            updateSubmitButtonState(false);
            alert(
              'Security verification failed. Please try again in a few minutes.'
            );
            return;
          }
        }
    ...
    if (submitBtn) {
      if (isSubmitting) {
        ...
        if (nameInput && phoneInput && emailInput) {
          ...
          submitBtn.innerHTML =
            'Schedule Service <svg xmlns="http://www.w3.org/2000/svg" width="9" height="16" viewBox="0 0 9 16" fill="none"><path d="M1 14.9231L7.47761 7.99998L1 1.0769" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  ...
  const resetBookItButton = () => {
    ...
    if (submitBtn) {
      submitBtn.innerHTML =
        'Book It <svg xmlns="http://www.w3.org/2000/svg" width="9" height="16" viewBox="0 0 9 16" fill="none"><path d="M1 14.9231L7.47761 7.99998L1 1.0769" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  ...
  window.submitFormWithValidation = () => {
    ...
    if (!hasErrors) {
      ...
      if (submitBtn) {
        submitBtn.innerHTML =
          'Booking... <svg xmlns="http://www.w3.org/2000/svg" width="9" height="16" viewBox="0 0 9 16" fill="none"><path d="M1 14.9231L7.47761 7.99998L1 1.0769" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  ...
  const applyCachedConfig = cachedConfig => {
    ...
    try {
      ...
      if (
        ...
        cfg.siteKey &&
        window.dhCaptcha &&
        typeof window.dhCaptcha.init === 'function'
      ) {
  ...
  window.proceedToQuoteWithValidation = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (submitBtn) {
          submitBtn.innerHTML =
            'See Your Quote <svg xmlns="http://www.w3.org/2000/svg" width="9" height="16" viewBox="0 0 9 16" fill="none"><path d="M1 14.9231L7.47761 7.99998L1 1.0769" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
          submitBtn.classList.remove('submitting');
          submitBtn.disabled = false;
        }
      }, 100); // Brief delay to ensure step transition is visible
  ...
  const getCheapestFullCoveragePlan = async () => {
    try {
      ...
      if (data.success && data.suggestions && data.suggestions.length > 0) {
        ...
        const fullCoveragePlans = data.suggestions.filter(
          plan =>
            plan.coverage_match &&
        ...
        if (fullCoveragePlans.length > 0) {
          ...
          const cheapestPlan = fullCoveragePlans.sort(
            (a, b) => parseFloat(a.initial_price) - parseFloat(b.initial_price)
          )[0];
  ...
  const recoverPartialLead = async (companyId, sessionId) => {
    try {
      ...
      if (!response.ok) {
        return {
          success: false,
          error: data.error || 'Failed to recover form data',
        };
      }
    ...
    try {
      ...
      progressiveFormManager = {
        ...
        loadLocalFormState: () => {
          try {
            const saved = localStorage.getItem(
              'dh_form_state_' + config.companyId
            );
        ...
        saveFormStateToLocalStorage: () => {
          try {
            ...
            localStorage.setItem(
              'dh_widget_progress_' + config.companyId,
        ...
        showFieldWarning: (field, message) => {
          ...
          if (container) {
            const existingWarnings =
              container.querySelectorAll('.dh-field-warning');
        ...
        showContinuePrompt: savedData => {
          ...
          const currentStepName =
            stepNames[savedData.currentStep] || 'your information';
          const timeAgo = getTimeAgo(new Date(savedData.timestamp));

          const primaryColor =
            (widgetState.widgetConfig &&
              widgetState.widgetConfig.colors &&
              widgetState.widgetConfig.colors.primary) ||
            '#3b82f6';
      ...
      const populateFormFields = () => {
        ...
        try {
          ...
          const addressStreet =
            data.addressStreet || data.addressDetails?.street || '';
          const addressCity =
            data.addressCity || data.addressDetails?.city || '';
          const addressState =
            data.addressState || data.addressDetails?.state || '';
          ...
          if (data.contactInfo) {
            const { name, firstName, lastName, email, phone } =
              data.contactInfo;
            ...
            if (firstName) {
              const firstNameInput =
                document.getElementById('quote-first-name-input') ||
            ...
            if (lastName) {
              const lastNameInput =
                document.getElementById('quote-last-name-input') ||
      ...
      if (containerId) {
        ...
        if (scriptTag.parentNode) {
          if (config.displayMode === 'button') {
            scriptTag.parentNode.insertBefore(elements, scriptTag.nextSibling);
          } else {
            scriptTag.parentNode.insertBefore(
              elements.formWidget,
              scriptTag.nextSibling
      ...
      if (urlParams.continue_session) {
        ...
        if (typeof recoverPartialLead === 'function') {
          try {
            serverRecoveryData = await recoverPartialLead(
              config.companyId,
              urlParams.continue_session
            );
          } catch (error) {
            console.warn(
              'Failed to recover cross-device partial lead from server:',
              error
            );
          }
        }
        ...
        if (
          serverRecoveryData &&
          serverRecoveryData.success &&
          serverRecoveryData.hasPartialLead
        ) {
      ...
      const savedData =
        progressiveFormManager.restoreFormStateFromLocalStorage();
      ...
      if (
        ...
        const localTimestamp = savedData
      ...
      if (
        recoveryData &&
        progressiveFormManager.shouldPromptToContinue(recoveryData)
      ) {
