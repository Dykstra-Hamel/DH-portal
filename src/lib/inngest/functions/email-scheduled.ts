...
export const emailScheduledHandler = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Step 3: Get email template and company info
    const emailData = await step.run('get-email-data', async () => {
      ...
      // Get company phone number from settings if available
      const { data: phoneSettings } = await supabase
        .from('company_settings')
        .select('setting_value')
        .eq('company_id', companyId)
        .eq('setting_key', 'company_phone');

      const companyPhone = phoneSettings?.[0]?.setting_value || '';
    ...
    // Step 5: Process email template with variables
    const processedEmail = await step.run('process-email-template', async () => {
      return processEmailTemplate(emailData.template, {
        ...variables,
