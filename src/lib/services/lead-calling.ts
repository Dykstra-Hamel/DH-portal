  ...
  try {
    ...
    const payload = {
      ...
      retell_llm_dynamic_variables: {
        ...
        customer_name:
          `${callData.customer.first_name || ''} ${callData.customer.last_name || ''}`.trim() ||
          'Customer',
        ...
        company_url:
          callData.company?.website || 'https://www.dykstrahamel.com/',
...
export async function shouldAutoCall(companyId: string): Promise<boolean> {
  try {
    ...
    if (!setting) {
      console.log(
        `No auto_call_enabled setting found for company ${companyId}, defaulting to enabled`
      );
      return true;
    }
...
export async function handleAutoLeadCall(
  ...
  if (!callingAllowed) {
    return {
      ...
      skipped: true,
      reason:
        'Calling not allowed due to business hours or weekend restrictions',
    };
  }
