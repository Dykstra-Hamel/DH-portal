...
export function FieldMapDashboard({
    ...
    : (() => {
        switch (activeFilter) {
          case 'total':
            return stops;
          case 'completed':
            return stops.filter(isCompleted);
          default:
            return stops.filter(s => !isCompleted(s)); // 'left'
        }
      })();
  ...
  useEffect(() => {
    ...
    nextIncompleteStop?.lat,
    nextIncompleteStop?.lng,
    nextIncompleteStop?.housePhotoUrl,
  ]);
      ...
      <div className={`${styles.body}`}>
        ...
        {!loading && !needsSetup && !error && stops.length > 0 && (
          <>
            ...
            <div className={styles.stopListContainerWrapper}>
