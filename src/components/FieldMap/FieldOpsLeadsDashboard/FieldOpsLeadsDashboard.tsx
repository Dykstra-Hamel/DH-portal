...
import styles from './FieldOpsLeadsDashboard.module.scss';

type LeadTab = 'new' | 'my' | 'closed';

interface FieldOpsLead {
  id: string;
  company_id: string;
  lead_status: string;
  service_type?: string;
...
function getAddressText(lead: FieldOpsLead): string {
...
interface FieldOpsLeadsDashboardProps {
  companyId: string;
  userId: string;
}

const BASE_COLUMNS: ColumnDefinition<FieldOpsLead>[] = [
  {
    key: 'created_at',
    title: 'In Queue',
    sortable: false,
...
export function FieldOpsLeadsDashboard({ companyId, userId }: FieldOpsLeadsDashboardProps) {
  const [activeTab, setActiveTab] = useState<LeadTab>('new');
  const [leads, setLeads] = useState<FieldOpsLead[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchCount = (type: LeadTab) => {
      ...
      return fetch(`/api/field-ops/leads?${params}`)
        .then(r => r.json())
  ...
  useEffect(() => {
    ...
    fetch(`/api/field-ops/leads?${params}`)
      .then(r => r.json())
      .then(d => {
