...
const getFormatIcon = (lead: Lead) => {
  ...
  const format = lead.format || (
    ...
    (lead.lead_type === 'email' || lead.lead_type === 'email_inbound' || lead.lead_type === 'campaign_email') ? 'email' :
    (lead.lead_type === 'campaign_text') ? 'text' :
    null
  );
  ...
  switch (format) {
    case 'call':
      return (
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="19" viewBox="0 0 18 19" fill="none" className={styles.phoneIcon}>
    ...
    case 'form':
      return (
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="19" viewBox="0 0 18 19" fill="none" className={styles.formIcon}>
    ...
    case 'email':
      return <Mail size={18} className={styles.emailIcon} color="#0088CC" />;
    case 'text':
    ...
    render: (lead: Lead) => (
      <div className={styles.formatCell} style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 4 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {getFormatIcon(lead)}
          <span className={styles.sourceBadge} style={{ whiteSpace: 'nowrap' }}>
