...
type EmailProvider = 'aws-ses' | 'mailersend';
...
export default function EmailDomainManager({ companyId }: EmailDomainManagerProps) {
  ...
  // ── Provider toggle ──────────────────────────────────────────────────────────
  const [activeProvider, setActiveProvider] = useState<EmailProvider>('aws-ses');
  const [selectedProvider, setSelectedProvider] = useState<EmailProvider>('aws-ses');

  // ── MailerSend state ─────────────────────────────────────────────────────────
  const [msHasApiKey, setMsHasApiKey] = useState(false);
  ...
  const fetchMailerSendSettings = useCallback(async () => {
    try {
      const res = await fetch(`/api/admin/companies/${companyId}/mailersend`);
      if (!res.ok) return;
      const data = await res.json();
      const provider: EmailProvider = data.provider === 'mailersend' ? 'mailersend' : 'aws-ses';
      setActiveProvider(provider);
      setSelectedProvider(provider);
      setMsHasApiKey(data.hasApiKey || false);
      setMsFromEmail(data.fromEmail || '');
      setMsFromName(data.fromName || '');
    } catch {
      // non-critical — fall back to aws-ses
    }
  }, [companyId]);

  const fetchTenantStatus = useCallback(async () => {
    try {
      const res = await fetch(`/api/admin/companies/${companyId}/provision-ses`);
      if (!res.ok) return;
      const data = await res.json();
      if (data.success) {
  ...
  const fetchDomain = useCallback(async () => {
    try {
      const res = await fetch(`/api/admin/companies/${companyId}/domain`);
      if (!res.ok) return;
      const data = await res.json();
  ...
  useEffect(() => {
    if (!companyId) return;
    setLoading(true);
    Promise.all([fetchMailerSendSettings(), fetchTenantStatus(), fetchDomain()]).finally(() =>
      setLoading(false)
    );
  }, [companyId, fetchMailerSendSettings, fetchTenantStatus, fetchDomain]);
  ...
  // ── MailerSend save ──────────────────────────────────────────────────────────
  const handleSaveMailerSend = async () => {
    if (!msFromEmail) {
      setError('From Email is required.');
      return;
    }
    if (!msApiKey && !msHasApiKey) {
    ...
    try {
      const body: Record<string, string> = {
        provider: 'mailersend',
        fromEmail: msFromEmail,
      };
      if (msApiKey) body.apiKey = msApiKey;
      if (msFromName) body.fromName = msFromName;
      ...
      setMsHasApiKey(true);
      setMsApiKey('');
      setSuccess('MailerSend settings saved. Click "Activate MailerSend" to start routing emails through it.');
    } catch (err) {
  ...
  // Switch tab only — does not change the active provider
  const handleProviderSelect = (provider: EmailProvider) => {
    setSelectedProvider(provider);
  };

  const handleActivateSES = async () => {
    setError(null);
    setSuccess(null);
    try {
      const res = await fetch(`/api/admin/companies/${companyId}/mailersend`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ provider: 'aws-ses' }),
      });
      if (!res.ok) throw new Error('Failed to activate AWS SES');
      setActiveProvider('aws-ses');
      setSuccess('AWS SES is now the active email provider.');
    } catch {
      setError('Failed to activate AWS SES.');
    }
  };

  const handleActivateMailerSend = async () => {
    if (!msHasApiKey) {
      setError('Save your MailerSend settings before activating.');
      return;
    }
    setError(null);
    setSuccess(null);
    try {
      const res = await fetch(`/api/admin/companies/${companyId}/mailersend`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ provider: 'mailersend', fromEmail: msFromEmail, fromName: msFromName }),
      });
      if (!res.ok) throw new Error('Failed to activate MailerSend');
      setActiveProvider('mailersend');
      setSuccess('MailerSend is now the active email provider.');
    } catch {
      setError('Failed to activate MailerSend.');
    }
  };
  ...
  const handleProvisionTenant = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to provision SES tenant');
  ...
  const handleDeleteTenant = async () => {
    if (!confirm('Are you sure you want to delete this SES tenant? This will remove all email sending configuration and cannot be undone.')) return;
    setDeletingTenant(true);
    setError(null);
    setSuccess(null);
    try {
      const res = await fetch(`/api/admin/companies/${companyId}/provision-ses`, { method: 'DELETE' });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to delete SES tenant');
  ...
  const handleAddDomain = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formDomain) { setError('Please enter a domain name'); return; }
    setSubmitting(true);
    setError(null);
    setSuccess(null);
    try {
      const res = await fetch(`/api/admin/companies/${companyId}/domain`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ domain: formDomain, emailPrefix: formPrefix }),
      });
      const data = await res.json();
  ...
  const handleDeleteDomain = async () => {
    if (!confirm('Are you sure you want to delete this domain configuration? This cannot be undone.')) return;
    setDeleting(true);
    setError(null);
    setSuccess(null);
    try {
      const res = await fetch(`/api/admin/companies/${companyId}/domain`, { method: 'DELETE' });
      const data = await res.json();
  ...
  const handleSendDnsInstructions = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/companies/${companyId}/email-dns-instructions`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ recipientEmail: emailRecipient, domain: domain?.name }),
      });
      const data = await res.json();
    ...
    <div className={styles.emailDomainManager}>
      <div className={styles.header}>
        ...
        <p className={styles.subtitle}>
          Choose an email provider and configure how this company sends outbound emails.
        </p>
      </div>
      ...
      <div className={styles.providerToggle}>
        <button
          ...
          onClick={() => handleProviderSelect('aws-ses')}
        >
          <span className={styles.providerTabLabel}>AWS SES</span>
        ...
        <button
          ...
          onClick={() => handleProviderSelect('mailersend')}
        >
          <span className={styles.providerTabLabel}>MailerSend</span>
          {activeProvider === 'mailersend' && (
            <span className={styles.activePill}>Active</span>
          )}
        </button>
      </div>
        ...
        <>
            ...
            <div className={styles.domainInfo}>
                        ...
                        <div className={styles.recordFields}>
                          <div className={styles.recordField}>
                            ...
                            <div className={styles.recordValue}>
                              ...
                              <button type="button" onClick={() => copyToClipboard(record.hostname)} className={styles.btnCopy} title="Copy to clipboard">Copy</button>
                            </div>
                          </div>
                          <div className={styles.recordField}>
                            <label>Value:</label>
                            <div className={styles.recordValue}>
                              ...
                              <button type="button" onClick={() => copyToClipboard(record.value)} className={styles.btnCopy} title="Copy to clipboard">Copy</button>
                            </div>
                          </div>
                          {record.priority && (
                            <div className={styles.recordField}>
              ...
              {domain.status === 'verified' && domain.verifiedAt && (
                <div className={styles.verifiedInfo}>
                  <p>&#10003; Domain verified on {new Date(domain.verifiedAt).toLocaleDateString()}</p>
                </div>
              )}

              <div className={styles.domainActions}>
                <button type="button" onClick={handleVerifyDomain} className={styles.btnPrimary} disabled={verifying || domain.status === 'verified'}>
                  {verifying ? 'Verifying...' : 'Verify Domain'}
                </button>
                {domain.status !== 'verified' && domain.records && domain.records.length > 0 && (
                  <button type="button" onClick={() => setShowEmailModal(true)} className={styles.btnSecondary} disabled={verifying || deleting || sendingEmail}>
                    Email DNS Instructions
                  </button>
                )}
                <button type="button" onClick={() => setIsEditing(true)} className={styles.btnSecondary} disabled={verifying || deleting}>
                  Change Domain
                </button>
                <button type="button" onClick={handleDeleteDomain} className={styles.btnDanger} disabled={verifying || deleting}>
          ...
          {activeProvider !== 'aws-ses' && (
            <div className={styles.activateRow}>
              <button
                type="button"
                onClick={handleActivateSES}
                className={styles.btnActivate}
              >
                Activate AWS SES
              </button>
              <span className={styles.activateHint}>AWS SES is not currently the active provider.</span>
            </div>
          )}
        </>
      )}
      ...
      {selectedProvider === 'mailersend' && (
        <div className={styles.mailerSendPanel}>
          <div className={styles.mailerSendInfo}>
            MailerSend uses your own API key and verified sender address — no domain provisioning or DNS setup required.
          </div>
          ...
          <div className={styles.formGroup}>
            ...
            <input
              ...
              placeholder={msHasApiKey ? '••••••••••••••••' : 'Enter your MailerSend API key'}
              className={styles.input}
              disabled={msSaving}
              autoComplete="off"
            />
            {msHasApiKey && (
              <small>An API key is already saved. Leave blank to keep it, or enter a new one to replace it.</small>
            )}
          </div>
          ...
          <div className={styles.formGroup}>
            ...
            <input
              ...
              id="ms-from-email"
              value={msFromEmail}
              onChange={e => setMsFromEmail(e.target.value)}
              placeholder="noreply@yourdomain.com"
              className={styles.input}
              disabled={msSaving}
            />
            <small>Must be a verified sender address in your MailerSend account.</small>
          </div>

          <div className={styles.formGroup}>
            <label htmlFor="ms-from-name">
              From Name <span className={styles.optionalLabel}>(optional)</span>
            </label>
            <input
              type="text"
              id="ms-from-name"
              value={msFromName}
              onChange={e => setMsFromName(e.target.value)}
          ...
          <div className={styles.formActions}>
            <button
              type="button"
              onClick={handleSaveMailerSend}
              className={styles.btnPrimary}
              disabled={msSaving}
            >
              {msSaving ? 'Saving...' : 'Save MailerSend Settings'}
            </button>
          </div>
          ...
          {activeProvider !== 'mailersend' && (
            <div className={styles.activateRow}>
              <button
                type="button"
                onClick={handleActivateMailerSend}
                className={styles.btnActivate}
                disabled={!msHasApiKey}
              >
                Activate MailerSend
              </button>
              <span className={styles.activateHint}>
                {msHasApiKey
                  ? 'MailerSend is not currently the active provider.'
                  : 'Save your settings first before activating.'}
              </span>
            </div>
          )}
        </div>
        ...
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              ...
              <button
                type="button"
                onClick={() => { setShowEmailModal(false); setEmailRecipient(''); setEmailError(null); }}
            ...
            <div className={styles.modalBody}>
              ...
              {emailError && (
                <div className={styles.alert} data-type="error">{emailError}</div>
              )}
              ...
              <div className={styles.formActions}>
                <button type="button" onClick={handleSendDnsInstructions} className={styles.btnPrimary} disabled={sendingEmail || !emailRecipient}>
                  {sendingEmail ? 'Sending...' : 'Send Instructions'}
                </button>
                <button type="button" onClick={() => { setShowEmailModal(false); setEmailRecipient(''); setEmailError(null); }} className={styles.btnSecondary} disabled={sendingEmail}>
                  Cancel
                </button>
              </div>
            </div>
