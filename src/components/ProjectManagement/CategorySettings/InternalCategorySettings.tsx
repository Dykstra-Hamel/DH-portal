...
export default function InternalCategorySettings() {
  ...
  const handleUpdate = async (categoryData: Partial<ProjectCategory>) => {
    ...
    const response = await fetch(`/api/admin/project-categories/${selectedCategory.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(categoryData),
    });
  ...
  const handleDeleteClick = async (category: ProjectCategory) => {
    ...
    try {
      const response = await fetch(`/api/admin/project-categories/${category.id}/projects`);
      if (response.ok) {
        const data = await response.json();
        setProjectsForCategory(data.projects || []);
      } else {
        setProjectsForCategory([]);
      }
    } catch (err) {
      console.error('Failed to fetch projects for category:', err);
      setProjectsForCategory([]);
    }
  ...
  const handleDrop = async (e: React.DragEvent, dropIndex: number) => {
    ...
    try {
      const response = await fetch('/api/admin/project-categories/reorder', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    <div className={styles.settingsContainer}>
      <div className={styles.section}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle}>Internal Project Categories</h2>
          <button
        ...
        {categories.length === 0 ? (
          <div className={styles.emptyState}>
            ...
            <p className={styles.emptyStateSubtext}>
              Create your first project category to get started
            </p>
          </div>
        ) : (
