...
import { ArrowLeft, Edit, Phone, Mail, MapPin, Building, Calendar, DollarSign, User as UserIcon, Save, X } from 'lucide-react'
...
export default function CustomerDetailPage({ params }: CustomerPageProps) {
    ...
    const [activeLeadsTab, setActiveLeadsTab] = useState<'active' | 'completed'>('active')
    const [isEditing, setIsEditing] = useState(false)
    const [editFormData, setEditFormData] = useState<CustomerFormData | null>(null)
                                        ...
                                        <div className={styles.contactItem}>
                                            ...
                                            <div>
                                                ...
                                                {(customer.city || customer.state) && (
                                                    <div>{[customer.city, customer.state, customer.zip_code].filter(Boolean).join(', ')}</div>
                                                )}
                                            </div>
                                        </div>
                                    )}
                            ...
                            <div className={styles.infoCard}>
                                ...
                                <div className={styles.customerDetails}>
                                    <div className={styles.detailItem}>
                                        <label>Status</label>
                                        <span className={`${styles.statusBadge} ${styles[customer.customer_status]}`}>
                                    ...
                                    <div className={styles.detailItem}>
                                        ...
                                        <span className={styles.totalValue}>{formatCurrency(customer.total_value || 0)}</span>
                                    </div>
                                </div>
                                {customer.notes && (
                        ...
                        <div className={styles.leadsTable}>
                            {displayLeads.map((lead) => (
                                <div 
                                    ...
                                    onClick={() => activeLeadsTab === 'active' ? handleLeadClick(lead.id) : undefined}
                                >
                                    <div className={styles.leadHeader}>
                                        <div className={styles.leadTitle}>
                                            <h4>{lead.service_type}</h4>
                                            <span className={styles.leadSource}>{lead.lead_source}</span>
                                        </div>
                                    ...
                                    <div className={styles.leadDetails}>
                                        <div className={styles.leadMeta}>
                                            ...
                                            {lead.assigned_user && (
                                                <div className={styles.metaItem}>
                                                    <UserIcon size={14} />
                                                    <span>Assigned to: {lead.assigned_user.first_name} {lead.assigned_user.last_name}</span>
                                                </div>
                                            )}
                                        </div>
                                        {lead.comments && (
