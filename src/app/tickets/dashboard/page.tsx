...
const SUPPORT_SERVICE_TYPES = new Set([
  'Customer Service',
  'customer service',
  'customer_service',
  'Support',
...
const getDashboardItemTypeLabel = (item: DashboardItem): string => {
  ...
  if (item.service_type === 'Sales' || item.service_type === 'sales') {
    return 'New Sales';
  }
  if (item.service_type && SUPPORT_SERVICE_TYPES.has(item.service_type)) {
    return 'New Support';
  }
  return 'New';
};
  ...
  const dashboardOverviewColumns = useMemo<ColumnDefinition<DashboardItem>[]>(
    () => [
      ...
      },
      {
        ...
        render: (item: DashboardItem) => {
          const label = getDashboardItemTypeLabel(item);
          const isSales = label === 'Sales' || label === 'New Sales';
          const isSupport = label === 'Support' || label === 'New Support';
          return (
            <div className={tableStyles.formatCell}>
              {isSales && (
                <svg
                  ...
                  viewBox="0 0 18 19"
                  fill="none"
                  className={tableStyles.salesIcon}
                >
                  <path
              ...
              {isSupport && (
                <svg
                  ...
                  viewBox="0 0 16 16"
                  fill="none"
                  className={tableStyles.supportIcon}
                >
                  <path
      ...
      },
      {
        ...
        render: (
          ...
          const label = item._type === 'ticket' ? 'Review Ticket' : 'View';
          return (
            <button
