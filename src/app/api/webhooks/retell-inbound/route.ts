...
async function sendCallSummaryEmailsIfEnabled(
  ...
  try {
    ...
    if (disconnectionReason === 'call_transfer' || 
        ...
        disconnectionReason?.toLowerCase().includes('transfer')) {
      console.log(`[Inbound Call Summary Emails] Skipping email notifications for call ${callId} - call was successfully transferred (reason: ${disconnectionReason})`);
      return;
    }
